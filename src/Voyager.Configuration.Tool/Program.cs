...
var keyEnvOption = new Option<string>("--key-env")
{
...
var decLegacyKeyOption = new Option<string?>("--legacy-key")
{
    Description = "Legacy DES key (for reading pre-v2 ciphertext; not recommended - use --legacy-key-env)"
};

var decLegacyKeyEnvOption = new Option<string?>("--legacy-key-env")
{
...
var encryptTextArg = new Argument<string>("text") { Description = "Text to encrypt" };
encryptValueCommand.Add(encryptTextArg);
encryptValueCommand.Add(keyOption);
encryptValueCommand.Add(keyEnvOption);
encryptValueCommand.SetAction(r =>
{
    var text = r.GetValue(encryptTextArg)!;
    var key = r.GetValue(keyOption);
...
var decryptTextArg = new Argument<string>("encrypted") { Description = "Encrypted text to decrypt" };
decryptValueCommand.Add(decryptTextArg);
decryptValueCommand.Add(keyOption);
decryptValueCommand.Add(keyEnvOption);
decryptValueCommand.Add(decLegacyKeyOption);
decryptValueCommand.Add(decLegacyKeyEnvOption);
decryptValueCommand.SetAction(r =>
{
    var encrypted = r.GetValue(decryptTextArg)!;
    var key = r.GetValue(keyOption);
    var keyEnv = r.GetValue(keyEnvOption)!;
    var legacyKey = r.GetValue(decLegacyKeyOption);
    var legacyKeyEnv = r.GetValue(decLegacyKeyEnvOption);
    try
    {
        var encryptionKey = GetEncryptionKey(key, keyEnv);
...
var encryptInputOption = new Option<FileInfo>("--input", "-i")
{
    Description = "Input plain JSON file",
    Required = true
};
var encryptOutputOption = new Option<FileInfo?>("--output", "-o")
{
...
encryptCommand.Add(encryptInputOption);
encryptCommand.Add(encryptOutputOption);
encryptCommand.Add(keyOption);
encryptCommand.Add(keyEnvOption);
encryptCommand.Add(forceOption);
encryptCommand.Add(inPlaceOption);

encryptCommand.SetAction(async (r, ct) =>
{
    var input = r.GetValue(encryptInputOption)!;
    var output = r.GetValue(encryptOutputOption);
    var key = r.GetValue(keyOption);
    var keyEnv = r.GetValue(keyEnvOption)!;
    var force = r.GetValue(forceOption);
    var inPlace = r.GetValue(inPlaceOption);
    try
    {
        if (!input.Exists)
        {
        ...
        // Read and parse JSON
        var jsonText = await File.ReadAllTextAsync(input.FullName, ct);
        ...
        await File.WriteAllTextAsync(outputFile.FullName, encryptedJson, ct);
...
var decryptInputOption = new Option<FileInfo>("--input", "-i")
{
    Description = "Input encrypted JSON file",
    Required = true
};
var decryptOutputOption = new Option<FileInfo>("--output", "-o")
{
    Description = "Output decrypted JSON file",
    Required = true
};

decryptCommand.Add(decryptInputOption);
decryptCommand.Add(decryptOutputOption);
decryptCommand.Add(keyOption);
decryptCommand.Add(keyEnvOption);
decryptCommand.Add(forceOption);
decryptCommand.Add(decLegacyKeyOption);
decryptCommand.Add(decLegacyKeyEnvOption);

decryptCommand.SetAction(async (r, ct) =>
{
    var input = r.GetValue(decryptInputOption)!;
    var output = r.GetValue(decryptOutputOption)!;
    var key = r.GetValue(keyOption);
    var keyEnv = r.GetValue(keyEnvOption)!;
    var force = r.GetValue(forceOption);
    var legacyKey = r.GetValue(decLegacyKeyOption);
    ...
    try
    {
        ...
        await File.WriteAllTextAsync(output.FullName, decryptedJson, ct);
...
var reencryptInputOption = new Option<FileInfo>("--input", "-i")
{
    Description = "Input JSON file to re-encrypt",
    Required = true
};
var legacyKeyEnvOption = new Option<string>("--legacy-key-env")
{
    Description = "Name of environment variable holding the legacy DES key (for reading old values)",
    DefaultValueFactory = _ => "ASPNETCORE_ENCODEKEY"
};
var newKeyEnvOption = new Option<string>("--new-key-env")
{
    Description = "Name of environment variable holding the new AES-256 key (for writing)",
    Required = true
};
...
reencryptCommand.Add(reencryptInputOption);
reencryptCommand.Add(legacyKeyEnvOption);
reencryptCommand.Add(newKeyEnvOption);
reencryptCommand.Add(dryRunOption);

reencryptCommand.SetAction(async (r, ct) =>
{
    var input = r.GetValue(reencryptInputOption)!;
    var legacyKeyEnv = r.GetValue(legacyKeyEnvOption)!;
    var newKeyEnv = r.GetValue(newKeyEnvOption)!;
    var dryRun = r.GetValue(dryRunOption);
    try
    {
        if (!input.Exists)
        {
        ...
        {
            var options = new JsonSerializerOptions { WriteIndented = true };
            await File.WriteAllTextAsync(input.FullName, result.ToJsonString(options), ct);
...
rootCommand.Add(encryptValueCommand);
rootCommand.Add(decryptValueCommand);
rootCommand.Add(keygenCommand);
rootCommand.Add(reencryptCommand);
